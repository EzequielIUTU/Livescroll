...
begin
  if not exists (select 1 from public.changelog_entries where display_version='6.2.4') then
    ...
    insert into public.changelog_entries(version,display_version,category,content) values
      ...
      (v_version,'6.2.4','mejora','Novedades ahora vive junto al selector y avisa con un punto cuando existen cambios sin mirar.'),
      (v_version,'6.2.4','mejora','El Panel de Admin incorpora la ficha moderna de usuarios y controles compartidos.'),
  ...
  if not exists (select 1 from public.changelog_entries where display_version='7.0.9') then
    ...
    insert into public.changelog_entries(version,display_version,category,content) values
      ...
      (v_version,'7.0.9','mejora','La identidad, la cuenta y el Panel de Admin se mantienen sincronizados entre generaciones.'),
      (v_version,'7.0.9','mejora','Novedades se consulta antes de entrar y recuerda individualmente lo que ya fue visto.'),
