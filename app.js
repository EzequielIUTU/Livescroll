...
const LIVESCROLL6_CLIENT_BUILD = 60203;
...
function ensureFeedPolishStyles() {
  ...
  style.textContent = `
    ...
    html.ls7-app-runtime:not(.ls8-app-runtime) #feedVertical,
    html.ls7-app-runtime:not(.ls8-app-runtime) #profileFeedVertical,
    html.ls7-app-runtime:not(.ls8-app-runtime) #foryouList .feed-vertical{
      scroll-snap-type:y proximity!important;scroll-behavior:auto!important;
      touch-action:pan-y!important;-webkit-overflow-scrolling:touch!important;
    }
    html.ls7-app-runtime:not(.ls8-app-runtime) #feedVertical>.feed-item,
    html.ls7-app-runtime:not(.ls8-app-runtime) #profileFeedVertical>.feed-item,
    html.ls7-app-runtime:not(.ls8-app-runtime) #foryouList .feed-vertical>.feed-item{
...
function setupOneVideoScroll(container) {
  ...
  if (LIVESCROLL_RUNTIME.isAndroid7) {
    container.dataset.oneVideoScroll = "natural";
    return;
  }
...
async function viewPublicProfile(username) {
  ...
  let { data: profile, error: profileError } = await sb.from("profiles").select("id, username, avatar_emoji, avatar_url, cover_url, cover_position_y, profile_side_image_url, bio, social_kick, social_twitch, social_youtube, social_tiktok, social_instagram, plan_id, is_live, live_platform, is_creator, is_creator_verified, kick_is_live, twitch_is_live, youtube_is_live, tiktok_is_live").eq("username", username).maybeSingle();

  // Compatibilidad con proyectos que todavía no aplicaron todas las columnas
  // sociales/de verificación. Una columna ausente no debe ocultar al usuario.
  if (profileError) {
    console.warn("Perfil público: usando consulta compatible", profileError.message);
    const fallbackResult = await sb.from("profiles")
      .select("id, username, avatar_emoji, avatar_url, cover_url, bio, social_kick, social_twitch, social_youtube, social_tiktok, social_instagram, plan_id, is_live, live_platform, is_creator")
  ...
  if (profileError) console.error("No se pudo cargar el perfil público", profileError);
...
/* LS8 one-time announcement · isolated from LS6/LS7 interface */
function installLS8DemoAnnouncementStyles(){if(document.getElementById("ls8DemoAnnouncementStyles"))return;const style=document.createElement("style");style.id="ls8DemoAnnouncementStyles";style.textContent=`.ls8-demo-announcement{position:fixed;z-index:2147483200;inset:0;display:grid;place-items:center;padding:18px;background:rgba(2,4,8,.82);backdrop-filter:blur(13px)}.ls8-demo-announcement-card{position:relative;width:min(520px,94vw);overflow:hidden;padding:34px;border:1px solid rgba(44,230,255,.42);border-radius:30px;background:radial-gradient(circle at 90% 0,rgba(44,230,255,.18),transparent 38%),radial-gradient(circle at 0 100%,rgba(255,61,131,.14),transparent 38%),#070b13;color:#fff;box-shadow:0 35px 110px rgba(0,0,0,.72),0 0 50px rgba(44,230,255,.08);font-family:Inter,system-ui,sans-serif}.ls8-demo-announcement-card:before{content:"";position:absolute;inset:0;opacity:.15;background-image:linear-gradient(rgba(44,230,255,.18) 1px,transparent 1px),linear-gradient(90deg,rgba(44,230,255,.18) 1px,transparent 1px);background-size:34px 34px;pointer-events:none}.ls8-demo-announcement-card>*{position:relative}.ls8-demo-announcement-card small{color:#c6ff3e;font:950 8px 'JetBrains Mono',monospace;letter-spacing:.2em}.ls8-demo-announcement-logo{margin:16px 0 12px;font-size:clamp(70px,18vw,118px);font-weight:1000;line-height:.78;letter-spacing:-.11em}.ls8-demo-announcement-logo b{color:#2ce6ff;text-shadow:0 0 30px rgba(44,230,255,.42)}.ls8-demo-announcement-card h2{margin:25px 0 8px;font-size:clamp(27px,7vw,45px);line-height:.98;letter-spacing:-.055em}.ls8-demo-announcement-card p{margin:0 0 25px;color:#a9b5c8;font-size:13px;line-height:1.55}.ls8-demo-announcement-actions{display:grid;grid-template-columns:1fr auto;gap:9px}.ls8-demo-announcement-actions button{min-height:49px;padding:12px 18px;border-radius:13px;font-weight:950}.ls8-demo-announcement-open{border:0;background:linear-gradient(100deg,#2ce6ff,#c6ff3e);color:#041014}.ls8-demo-announcement-later{border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.05);color:#aeb8c9}.ls8-demo-announcement-date{display:flex;align-items:center;justify-content:space-between;margin-top:19px;padding-top:16px;border-top:1px solid rgba(255,255,255,.10)}.ls8-demo-announcement-date span{color:#7f8b9e;font-size:9px;font-weight:900;letter-spacing:.12em}.ls8-demo-announcement-date b{color:#2ce6ff;font-size:17px}@media(max-width:500px){.ls8-demo-announcement-card{padding:27px 22px;border-radius:24px}.ls8-demo-announcement-actions{grid-template-columns:1fr}.ls8-demo-announcement-later{min-height:43px!important}}`;document.head.appendChild(style)}
